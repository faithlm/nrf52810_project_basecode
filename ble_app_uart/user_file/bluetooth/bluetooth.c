...
#define DEVICE_NAME                     "XT_JH_SLAVE"                               /**< Name of device. Will be included in the advertising data. */
...
BLE_CONFIG_DEF(m_config, NRF_SDH_BLE_TOTAL_LINK_COUNT);
...
static void services_init(void)
{
    ...
    config_init.data_handler = config_data_handler;

    err_code = ble_config_init(&m_config, &config_init);
    APP_ERROR_CHECK(err_code);
}
...
uint32_t user_bluetooth_init(void)
{
    ...
    conn_params_init();
    advertising_start();
    NRF_LOG_INFO("Bluetooth init finish & adv start");
}